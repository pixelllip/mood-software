...
class AI_Agent(QThread):
    ...
    def _compact_student_profile_for_schedule(self, student_profile: dict, *, weak_n: int = 4) -> dict:
        """
    ...
    def _extract_usage_tokens(self, resp) -> tuple[int | None, int | None, int | None]:
        """
    ...
    def use_tool(self, tool_name, arguments=None):
        ...
        if tool_name == "get_local_backlog":
            ...
            info = self.tool.get_traffic(**arguments)
            final_response = self.client.responses.create(
                model="qwen3.5-flash",
                input=[{
                    "role": "system",
